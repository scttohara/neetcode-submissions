class Solution:
    def productExceptSelf(self, nums: List[int]) -> List[int]:
        ...
        #creates postfix product array
        second_current_index = len(nums) - 1
        postfix_product_array = [0] * len(nums)
        while second_current_index > -1:
            if second_current_index == len(nums) - 1:
                postfix_product_array[second_current_index] = nums[second_current_index]
            else:
                postfix_product_array[second_current_index] = nums[second_current_index] * postfix_product_array[second_current_index + 1]
        ...
        while last_current_index < len(nums):
            ...
            if last_current_index == 0:
                array_to_return[last_current_index] = 1 * postfix_product_array[last_current_index + 1]
            elif last_current_index + 1 == len(nums):
                array_to_return[last_current_index] = prefix_product_array[last_current_index - 1] * 1
            else:
                array_to_return[last_current_index] = prefix_product_array[last_current_index - 1] * postfix_product_array[last_current_index + 1]
